#!/bin/bash -eu
# ClusterFuzzLite build script for mdview's Swift fuzz targets.
# Mirrors Fuzz/oss-fuzz/build.sh but resolves the source path from $SRC (the repo
# is copied to $SRC/mdview by .clusterfuzzlite/Dockerfile). Each fuzz target is a
# library source compiled with swiftc; libFuzzer supplies main() via
# -sanitize=fuzzer,address. A thin LLVMFuzzerTestOneInput shim is injected per
# target so the library files stay @_silgen_name-free.
